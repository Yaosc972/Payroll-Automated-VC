...
@pytest.mark.parametrize("kind,expected", [
    ("sheet", "sheet_confirmation_required"),
    ("jinjiang", "jinjiang_roster_confirmation_required"),
    ("shift", "night_shift_configuration_required"),
])
def test_direct_upload_enforces_new_confirmation_gates(monkeypatch, tmp_path, kind, expected):
    ...
    config = {"shift_breaks": [], "jinjiang_exclusions": [], "jinjiang_list_confirmed": False}
    monkeypatch.setattr(api, "load_night_shift_config", lambda *a, **kw: config)
    monkeypatch.setattr(api, "build_night_shift_config_snapshot", lambda value: value)
    class Loader:
        monthly = SimpleNamespace(rows=[{"工号": "QA", "工作地区": "晋江", "岗位名称": "操作员"}])
        def __init__(self, *args, **kwargs):
            assert kwargs["sheet_mapping"] == {"0:月考勤": "monthly"}
        def __enter__(self): return self
        def __exit__(self, *args): pass
        def validate_inputs(self, *args):
            if kind == "sheet":
                raise api.SheetConfirmationRequired([{"key": "0:月考勤", "candidates": ["monthly", "daily"]}])
            return {}
        def group_daily_by_employee(self):
            return {"QA": [{"出勤日期": "2026-07-02", "班次编号": "QA_MISSING", "工作地区": "晋江", "上班一": "20:00", "下班一": "08:00"}]}
    monkeypatch.setattr(api, "MultiFilePayrollDataLoader", Loader)
    run = api.create_payroll_run({"uploadMode": "direct", "status": "等待上传", "savedFileName": "qa.xlsx", "expectedFileSize": 4})
    path = api.get_payroll_run_dir(run["id"]) / "qa.xlsx"
    path.write_bytes(b"test")
    monkeypatch.setattr(api, "materialize_payroll_file", lambda *args: path)
    response = TestClient(app).post(f"/api/domestic-labor/runs/{run['id']}/direct-upload-complete", json={
        "engines": ["yeban_butie"], "attendanceMonth": "202607",
        "sheetMapping": {"0:月考勤": "monthly"},
        "jinjiangRosterDecision": "confirmed" if kind == "shift" else "",
    })
...
def test_missing_night_shift_returns_409_before_worker_and_removes_unsubmitted_run(monkeypatch, tmp_path, monthly_area, daily_area, decision, code):
    ...
    monkeypatch.setattr(api, "MultiFilePayrollDataLoader", Loader)
    calls = []
    monkeypatch.setattr(api, "_run_payroll_calculation", lambda *a, **kw: (calls.append(kw), {"status": "已完成"})[1])
    ...
    if code is None:
        assert response.status_code == 200
        assert len(calls) == 1
        assert calls[0]["night_shift_config"]["jinjiang_list_confirmed"] is True
        assert calls[0]["night_shift_config"]["jinjiang_exclusions"][0]["employee_id"] == "OTHER"
        return
