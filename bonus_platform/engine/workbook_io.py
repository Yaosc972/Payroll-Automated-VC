...
def build_final_workbook(base_path: Path, confirmation_path: Path, output_path: Path) -> None:
    ...
    for sheet_name in ("最终招聘奖金汇总", "最终内推奖金汇总", "确认留痕"):
        if sheet_name in workbook.sheetnames:
            del workbook[sheet_name]

    _write_summary_sheet(workbook, "最终招聘奖金汇总", recruitment_headers, recruitment_rows, table_name="tbl_final_recruitment_summary")
    _write_summary_sheet(workbook, "最终内推奖金汇总", referral_headers, referral_rows, table_name="tbl_final_referral_summary")
...
def _write_detail_sheet(workbook: Workbook, details: List[RecruitmentDetail]) -> None:
    ...
    _apply_date_format(sheet, [13, 14, 15])
    _apply_money_format(sheet, [21, 22, 23, 25, 27, 29, 31, 32, 34, 36, 38, 46, 47, 49, 51, 53])
    sheet.column_dimensions["E"].width = 24
    sheet.column_dimensions["J"].width = 18
    sheet.column_dimensions["L"].width = 18
    sheet.column_dimensions["BC"].width = 40
...
def _write_summary_sheet(
    ...
    _format_sheet(sheet, table_name=table_name, exception=exception)
    if not exception:
...
def _format_sheet(sheet, table_name: str, exception: bool = False) -> None:
    max_row = max(sheet.max_row, 1)
    max_col = max(sheet.max_column, 1)
    header_fill = EXCEPTION_FILL if exception else HEADER_FILL
