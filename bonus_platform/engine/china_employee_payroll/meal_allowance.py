...
def _wx_workbook_rows_from(path: Path, *, read_only: bool) -> tuple[list[str], list[dict[str, Any]], dict[str, Any]]:
    ...
    try:
        ...
        first_header = [_clean(value) for value in next(sheet.iter_rows(min_row=1, max_row=1, values_only=True), ())]
        second_header = [_clean(value) for value in next(sheet.iter_rows(min_row=2, max_row=2, values_only=True), ())]
        headers = [
