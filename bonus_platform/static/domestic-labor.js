...
async function submitPayrollWithRosterConfirmation(options, batch) {
  const key = JSON.stringify([batch.id, batch.month, (state.payrollFiles?.length ? state.payrollFiles : [state.payrollFile]).filter(Boolean).map(file=>[file.name,file.size,file.lastModified]), JSON.stringify(options.sheetMapping)]);
  if (state.jinjiangRosterConfirmedKey === key) options.jinjiangRosterDecision = 'confirmed';
  try {
    return await submitDomesticLaborRun(options);
  } catch (error) {
    ...
    if (choice === 'continue') {
      ...
      state.jinjiangRosterUploadKey = null;
      options.jinjiangRosterDecision = 'confirmed';
      return error.retryCompletion
        ? await error.retryCompletion({jinjiangRosterDecision: 'confirmed'})
        : await submitDomesticLaborRun(options);
    }
    if (choice === 'upload') {
...
async function submitCanbuBatch() {
  ...
  try {
    ...
    const data = await submitPayrollWithRosterConfirmation({
      ...
      password: el.filePassword?.value || '',
      hrbpList,
      sheetMapping: Object.fromEntries(sheetChoices.map(select => [select.dataset.sheetKey, select.value])),
...
async function submitDomesticLaborRun({ file, files = [], engines, attendanceMonth, password, hrbpList, sheetMapping = {}, jinjiangRosterDecision = '', statusElement, progressButton, onPlanCreated }) {
  ...
  try {
    ...
    return submitDomesticLaborRunMultipart({ files: selectedFiles, engines, attendanceMonth, password, hrbpList, sheetMapping, jinjiangRosterDecision });
  }
}

async function submitDomesticLaborRunDirect({ files, engines, attendanceMonth, password, hrbpList, sheetMapping, jinjiangRosterDecision, statusElement, progressButton, onPlanCreated }) {
  ...
  const completion = { engines, attendanceMonth, password, hrbpList, sheetMapping, jinjiangRosterDecision };
  const complete = updates => requestJson(`/api/domestic-labor/runs/${plan.runId}/direct-upload-complete`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ...completion, ...updates }),
  });
  try { return await complete(); }
  catch (error) { error.retryCompletion = complete; throw error; }
}
...
function submitDomesticLaborRunMultipart({ files, engines, attendanceMonth, password, hrbpList, sheetMapping, jinjiangRosterDecision }) {
  const form = new FormData();
