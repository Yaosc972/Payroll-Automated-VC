...
10. Worker 发布改为双平台原子门禁：macOS DMG 或 Windows EXE 单独上传时只进入待发布区，不提升最低版本；两个平台安装包版本一致后才一次性生效。
11. 新增 `.github/workflows/labor-worker-release.yml`，在原生 ARM64 macOS 与 x64 Windows Runner 上分别构建同版本安装包，最终任务要求两个文件同时存在；海外劳务 CI 也会在两个平台运行 Worker 测试，并阻止 Worker 运行时代码修改后未升级版本号。

## 已完成验证

```text
Python 相关回归：150 passed
...
应用内部 `codesign --verify --deep --strict` 通过，但当前没有 Developer ID 身份，`spctl` 拒绝。产品决策是小范围内部使用阶段不购买 Apple Developer Program：允许发布未公证安装包，Mac 用户首次打开按“系统设置 → 隐私与安全性 → 仍要打开”手动放行；Windows 未签名安装包按“更多信息 → 仍要运行”手动放行。网页必须保留首次打开说明，并以版本号和 SHA-256 作为下载核验依据。

## 发布窗口执行顺序

1. 只集成本交接列出的连接稳定性及双平台发布文件，保留工作区其他脏改动。
2. 部署服务端和网页代码，但先不要发布 `0.3.14` 安装包。
3. 运行 `Labor Worker Dual-Platform Build` 工作流，输入 `0.3.14`；必须同时取得 macOS ARM64 DMG、Windows x64 EXE 和 `SHA256SUMS`。
4. 在干净的普通用户环境分别验证 Mac“仍要打开”和 Windows“仍要运行”路径可用；当前小范围内部发布不以 Apple Developer ID、公证或 Windows Authenticode 签名为阻塞条件。
5. 复核文件名必须分别为 `Σ海外报账核对助手-0.3.14-arm64.dmg` 和 `Σ海外报账核对助手-0.3.14-windows-x64.exe`，重新计算 SHA-256。
6. 管理员通过海外劳务页面依次上传两个平台安装包。第一个只显示“已安全暂存”；第二个同版本安装包确认后，系统才一次性发布并把最低要求提升到 `0.3.14`。
7. 一台旧版机器升级后重新激活一次。旧的 8 小时凭据若已经过期，无法通过滑动续期复活；重新激活后将进入新的 30 天滑动续期机制。
8. 生产验证：
...
9. 对照 Worker 本地日志和 Vercel Production 日志，确认请求到达、状态码正常、无连续超时。

## 尚未验证

- 未在干净的普通 Mac/Windows 用户环境完成未签名安装包的首次打开实机验收。
- 未部署服务端或网页代码。
- 未上传安装包或激活新的生产发布清单。
