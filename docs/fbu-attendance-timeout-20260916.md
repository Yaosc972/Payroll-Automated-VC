...
- 第一版验证阶段尚未进行生产迁移、生产发布和真实页面重试；后续生产结果见下一节。

## 生产复测与第二版修复

- 第一版于 2026-09-17 上线后，真实上月考勤仍返回 `57014`；日志确认已调用新版 raw RPC，故问题不是迁移遗漏或兼容回退。
- 同一活动的旧入口历史最慢达到 7,875.9 ms，接近 8 秒上限；四次失败跨越约五小时，排查时没有活动锁等待，不支持“单次偶发锁”解释。
- 失败请求约 5.69 MB；其中考勤区块约 4.73 MB，13,034 条日明细反复携带字段名占主要体积。
- 第二版仅改变数据库存储传输形式：将日明细编码为一次字段表头加值数组，读取时无损还原。业务运行时、Storage 备份和页面仍使用原字典结构，历史未编码数据可继续读取。
- 使用当前生产只读快照与原 7 月文件重建同一输入：完整请求降至 2.40 MB，考勤区块由 4.81 MB 降至 1.53 MB；临时表调用约 470.9 ms，保存成功且解码后与原数据完全一致。临时对象测试未修改生产活动。
- 第二版 FBU 全量测试：382 passed，2 条依赖弃用警告。
- 第二版只需发布应用代码，不需要新增数据库迁移；生产真实文件仍须在发布后经操作者确认再重试一次。
...
1. 集成第二版代码提交；不要重复执行第一版数据库迁移。
2. 运行 `python -m pytest tests/test_fbu*.py -q`，并确认既有未编码考勤可以读取。
3. 发布应用后确认生产仍使用 `sigma_fbu_commit_snapshot_raw`，数据库函数和权限保持不变。
4. 经操作者确认后，仅重试活动 `556722ff` 中已保留的失败文件一次；确认解析成功、96 工时上下文完整、刷新后数据一致且没有新的 `57014`。
5. 结果不明确时先读取活动和任务状态，不连续重试，也不重传当月考勤或补充假勤。
