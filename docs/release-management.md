...
当前项目需要保留两条发布线，并将验证后的稳定版本同步到默认分支：

- Vercel 生产线：面向当前公网试用环境，负责首页、后台管理、权限、已开放模块。
- 公司服务器生产线：面向 IT 部署的公司内网/服务器环境，负责公司部署栈适配。
- GitHub 默认分支 `main`：作为公开、稳定的代码入口；不直接替代生产发布线，生产验证通过后同步。
...
| 用途 | 分支 | 说明 |
|---|---|---|
| 默认稳定分支 | `main` | GitHub 默认入口。保存已完成生产验证的稳定代码和对外 README；不得长期落后于当前生产版本。 |
...
生产验证完成后，将发布提交正常合并或快进到 `main` 并推送。禁止强制覆盖 `main`；如果 `main` 有独立提交，先合并并检查冲突，保留有效的 CI、文档和仓库配置。
